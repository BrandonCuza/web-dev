...
let _validate__id = (_id) => {
	return new Promise((resolve, reject) => {
		let is_valid = (validator.contains(_id, 'ObjectId(') && validator.contains(_id, ')') && validator.isLength(_id, {min:36, max:36}));
		if (is_valid){
			resolve('The id is valid.');
		} else {
...
export function validate_fields(_id, iso_code, continent, location, date, total_cases, new_cases, total_deaths, new_deaths) {
	return Promise.all([_validate__id(_id), _validate_iso_code(iso_code), _validate_continent(continent), _validate_location(location), _validate_date(date),
