...
describe('Canada Covid Database App v1 - Tests with Mocha', function(){
    describe('Test Models', function(){
        describe('Entry', function(){
            let c_id = 'ObjectId(\'640a2343a1210de9350bf5cc\')';
            let ciso= 'CAN';
            ...
            it('Test if entry is invalid function (Invalid ID)', async function(){
                let c = new Entry('Bad ID', ciso, ccont, cloc, cdate, ct_c, cn_c, ct_d, cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid ISO code)', async function(){
                let c = new Entry(c_id, 'Bad ISO', ccont, cloc, cdate, ct_c, cn_c, ct_d, cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid continent)', async function(){
                let c = new Entry(c_id, ciso, 'Bad Continent', cloc, cdate, ct_c, cn_c, ct_d, cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid location)', async function(){
                let c = new Entry(c_id, ciso, ccont, 'Bad Location', cdate, ct_c, cn_c, ct_d, cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid date)', async function(){
                let c = new Entry(c_id, ciso, ccont, cloc, 'Bad Date', ct_c, cn_c, ct_d, cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid number of total cases)', async function(){
                let c = new Entry(c_id, ciso, ccont, cloc, cdate, 'Bad Total Cases', cn_c, ct_d, cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid number of new cases)', async function(){
                let c = new Entry(c_id, ciso, ccont, cloc, cdate, ct_c, 'Bad New Cases', ct_d, cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid number of total deaths)', async function(){
                let c = new Entry(c_id, ciso, ccont, cloc, cdate, ct_c, cn_c, 'Bad Total Deaths', cn_d);
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
            it('Test if entry is invalid function (Invalid number of new deaths)', async function(){
                let c = new Entry(c_id, ciso, ccont, cloc, cdate, ct_c, cn_c, ct_d, 'Bad New Deaths');
                strictEqual(await validate_fields(c._id, c.iso_code, c.continent, c.location, c.date, c.total_cases, c.new_cases, c.total_deaths, c.new_deaths), false);
            });
        });
    });
    describe('Test API calls', function(){
        describe('Entries', async function(){            
            it('Fail 1. POST - Test invalid ID in the object', async function(){
                let data = {
                    _id: 'Bad ID', 
                    iso_code: 'CAN', 
            ...
            it('Fail 2. POST - Test invalid ISO code in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'Bad ISO', 
                    continent: 'North America', 
                    location: 'Canada',
                    date: '2018-08-06',
            ...
            it('Fail 3. POST - Test invalid continent in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'CAN', 
            ...
            it('Fail 4. POST - Test invalid location in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'CAN', 
                    continent: 'North America', 
            ...
            it('Fail 5. POST - Test invalid date in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'CAN', 
                    continent: 'North America', 
            ...
            it('Fail 6. POST - Test invalid number of total cases in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'CAN', 
                    continent: 'North America', 
            ...
            it('Fail 7. POST - Test invalid number of new cases in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'CAN', 
                    continent: 'North America', 
            ...
            it('Fail 8. POST - Test invalid number of total deaths in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'CAN', 
                    continent: 'North America', 
            ...
            it('Fail 9. POST - Test invalid number of new deaths in the object', async function(){
                let data = {
                    _id: 'ObjectId(\'640a2343a1210de9350bf5cc\')', 
                    iso_code: 'CAN', 
                    continent: 'North America', 
            ...
            it('Fail 12. PUT - /canada/:date (No entry with ID)', async function(){
                ...
                let res = await instance.put('/canada/'+data.date, data);
                strictEqual(res.data,'The new entry data is not valid.');
            });
            it('Success 1. POST - Valid entry, PUT - Valid data, GET - entry, DELETE - entry', async function(){
                ...
                strictEqual(res_get.data._id, data2._id);
