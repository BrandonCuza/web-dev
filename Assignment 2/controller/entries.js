...
export async function create(req, res) {
    ...
    let isValid = await validate_fields(_id, iso_code, continent, location, date, total_cases, new_cases, total_deaths, new_deaths);
    if (isValid){
        let new_entry = new Entry(_id, iso_code, continent, location, date, total_cases, new_cases, total_deaths, new_deaths);
...
export async function update_entry(req, res) {
    ...
    let isValid = await validate_fields(_id, iso_code, continent, location, date, total_cases, new_cases, total_deaths, new_deaths);
    if (isValid){
        let msg = await Entry.update(date_to_match, new Entry(_id, iso_code, continent, location, date, total_cases, new_cases, total_deaths, new_deaths));
        res.send(msg);
    } else {
